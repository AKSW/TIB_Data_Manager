...
if [ ! -e "$CONFIG" ]; then
  ...
  ckan -c "$CONFIG" config-tool "$CONFIG" -e \
     "ckan.plugins = stats text_view image_view recline_view resource_proxy datastore datapusher webpage_view   harvest ckan_harvester dcat dcat_json_interface dcat_rdf_harvester dcat_json_harvester structured_data STREAMtheme qualityreports"
